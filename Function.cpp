...
void Function::compute_live_var() {
	...
	for (int d = _myBB.size() - 1; d >= 0; d--) {
		auto bb = get_BB(d);

		// calcul de LiveOut
		for (int j = 0; j < bb->get_nb_succ(); j++) {
			for (int k = 0; k < NB_REG; k++) {
				bb->LiveOut[k] = bb->LiveOut[k]
						|| bb->get_successor(j)->LiveIn[k];
				//		cout<<"LiveOut : "<<bb->LiveOut[k]<<endl;
			}
		}
		...
		for (int i = 0; i < NB_REG; i++) {
			if (bb->Def[i] && bb->LiveOut[i]) {
				//	cout<<"def : "<<bb->Def[i]<<endl;
				bb->LiveIn[i] = bb->Use[i] || false;
			} else if (bb->LiveOut[i]) {
				//cout<<"use : "<<bb->Use[i]<<endl;
				bb->LiveIn[i] = true;
			} else {
